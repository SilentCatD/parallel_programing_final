...
void seamOnDeivce(unsigned char* inPixels, int width, int height, unsigned char* outPixels, dim3 blockSize = dim3(1, 1)){
	...
	// xSombel
	convoImageKernel<<<gridSize, blockSize, s_inPixelsSize>>>(d_outGrayScale, width, height, FILTER_WIDTH, d_xSombelOut, true);

	// ySombel	
	convoImageKernel<<<gridSize, blockSize, s_inPixelsSize>>>(d_outGrayScale, width, height, FILTER_WIDTH, d_ySombelOut, false);
