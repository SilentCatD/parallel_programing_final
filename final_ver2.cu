...
void findSeamOnDeivce(unsigned char* inPixels, int width, int height, int* deviceSeamPos, int* outCostTable, int* outPathTable, int &outMinColIdx, dim3 convoBlockSize = dim3(1, 1), int costTableBlockSize = 1024, int minColIdxBlockSize = 512){
	...
	// xSombel
	convoImageKernel<<<gridSize, convoBlockSize, s_inPixelsSize>>>(d_outGrayScale, width, height, FILTER_WIDTH, d_xSombelOut, true);

	// ySombel	
	convoImageKernel<<<gridSize, convoBlockSize, s_inPixelsSize>>>(d_outGrayScale, width, height, FILTER_WIDTH, d_ySombelOut, false);
